...
describe('UInt64', () => {
  ...
  it('wraps around like a C uint64_t on overflow', () => {
    expect(UInt64.of(18446744073709551615n).add(UInt64.of(1n))).toEqual(UInt64.of(0n));
  });

  it('subtracts, wrapping around on underflow like a C uint64_t', () => {
    expect(UInt64.of(5n).sub(UInt64.of(3n))).toEqual(UInt64.of(2n));
    expect(UInt64.of(0n).sub(UInt64.of(1n))).toEqual(UInt64.of(18446744073709551615n));
  });

  it('multiplies, wrapping around on overflow like a C uint64_t', () => {
    expect(UInt64.of(5n).mul(UInt64.of(3n))).toEqual(UInt64.of(15n));
    expect(UInt64.of(18446744073709551615n).mul(UInt64.of(2n))).toEqual(UInt64.of(18446744073709551614n));
  });
  ...
  it('compares with greaterThan/lessThan/greaterThanOrEqual/lessThanOrEqual', () => {
    ...
    expect(UInt64.of(5n).greaterThanOrEqual(UInt64.of(5n))).toEqual(Bool.of(true));
  ...
  it('flips every bit with bitwiseNot', () => {
    expect(UInt64.of(5n).bitwiseNot()).toEqual(UInt64.of(18446744073709551610n));
  });
  ...
  it('shifts left, wrapping around on overflow like a C uint64_t', () => {
    ...
    expect(UInt64.of(9223372036854775808n).shiftLeft(UInt64.of(1n))).toEqual(UInt64.of(0n));
  });

  it('shifts right logically, filling with zeros — unlike an arithmetic shift, which would keep this value unchanged', () => {
    expect(UInt64.of(18446744073709551615n).shiftRight(UInt64.of(1n))).toEqual(UInt64.of(9223372036854775807n));
  });
});
