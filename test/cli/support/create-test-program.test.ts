...
describe('createTestProgram', () => {
  it('builds a program containing the given virtual files, with real parseable content', () => {
    ...
    try {
      ...
      const nonLibFiles = program.getSourceFiles().filter((file) => !file.isDeclarationFile && !file.fileName.includes('node_modules'));
  ...
  it('supports additional real root files alongside virtual ones', () => {
    const realFilePath = fileURLToPath(new URL('./create-test-program.ts', import.meta.url));
