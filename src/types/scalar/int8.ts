...
/**
 * OpenCL's `char` — an 8-bit signed integer (-128 to 127). Arithmetic wraps around on
 * overflow, exactly like it would in C, rather than growing into a bigger JS number.
 */
export class Int8 extends Scalar<number> {
  /**
   * @param raw - The raw value; `wrap()` truncates it to the -128..127 range.
   */
  ...
  /**
   * Builds an `Int8` from a raw JS number, truncating it to the -128..127 range.
   *
   * @param raw - The raw JS number to wrap.
   * @returns The resulting `Int8`.
   */
  ...
  /**
   * Truncates `raw` to the -128..127 range.
   *
   * In plain terms: assigning a too-large number into a one-element `Int8Array` truncates
   * it exactly the way a C `int8_t` would — this scratch-array trick reproduces C's
  ...
  /**
   * Adds `other` to this value, wrapping around on overflow like C's `int8_t`.
   *
  ...
  /**
   * Subtracts `other` from this value, wrapping around on underflow like C's `int8_t`.
   *
  ...
  /**
   * Multiplies this value by `other`, wrapping around on overflow like C's `int8_t`.
   *
  ...
  /**
   * The remainder of dividing this value by `other`, with the sign of this value — C's `%`,
   * not a mathematical modulo.
   *
   * @param other - The divisor.
  ...
  /**
   * The negation of this value, wrapping like C's `int8_t` (the most negative value negates
   * to itself).
   *
